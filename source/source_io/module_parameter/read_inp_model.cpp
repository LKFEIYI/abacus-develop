...
* false: No potential barrier is added.)";
    ...
    }
    {
        ...
        item.check_value = [](const Input_Item& item, const Parameter& para) {
            ...
            if (para.input.solvation_model == "sccs"
                && para.input.calculation != "scf"
                && para.input.calculation != "relax")
            {
...
#undef ADD_SCCS_REAL_ITEM
    {
        ...
        item.check_value = [](const Input_Item& item, const Parameter& para) {
            if (!std::isfinite(para.input.sccs_start_drho)
                || para.input.sccs_start_drho < 0.0)
            {
                ModuleBase::WARNING_QUIT(
                    "ReadInput",
    ...
    }
    {
        ...
        item.check_value = [](const Input_Item& item, const Parameter& para) {
            if (para.input.sccs_start_nmax <= 0
                || (para.input.sccs_start_drho > 0.0
                    && para.input.sccs_start_nmax >= para.input.scf_nmax))
            {
                ModuleBase::WARNING_QUIT(
                    "ReadInput",
