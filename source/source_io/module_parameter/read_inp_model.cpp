...
* false: No potential barrier is added.)";
    ...
    }
    {
        ...
        item.category = "Implicit solvation model";
        item.type = "String";
        item.description = "Select legacy or the native SCCS implementation. SCCS is enabled only together with imp_sol=true.";
        item.default_value = "legacy";
        item.unit = "";
        read_sync_string(input.solvation_model);
        item.check_value = [](const Input_Item& item, const Parameter& para) {
            const std::vector<std::string> allowed = {"legacy", "sccs"};
            if (std::find(allowed.begin(), allowed.end(), para.input.solvation_model) == allowed.end())
            {
                ModuleBase::WARNING_QUIT("ReadInput", nofound_str(allowed, "solvation_model"));
            }
            if (para.input.solvation_model == "sccs" && !para.input.imp_sol)
            {
                ModuleBase::WARNING_QUIT("ReadInput", "solvation_model=sccs requires imp_sol=true");
            }
            if (para.input.solvation_model == "sccs"
                && (para.input.efield_flag || para.input.gate_flag
                    || para.input.assume_isolated != "none"))
            {
                ModuleBase::WARNING_QUIT("ReadInput",
                                         "SCCS cannot be combined with electric/gate fields or assume_isolated");
            }
            if (para.input.solvation_model == "sccs" && para.input.nspin == 4)
            {
                ModuleBase::WARNING_QUIT("ReadInput", "the first SCCS implementation does not support nspin=4");
            }
            if (para.input.solvation_model == "sccs" && para.input.calculation != "scf")
            {
                ModuleBase::WARNING_QUIT("ReadInput", "the first SCCS implementation supports only calculation=scf");
            }
            if (para.input.solvation_model == "sccs" && para.input.device == "gpu")
            {
                ModuleBase::WARNING_QUIT("ReadInput", "the first SCCS implementation supports only device=cpu");
            }
            if (para.input.solvation_model == "sccs" && para.input.dfthalf_type != 0)
            {
    ...
    }
    {
        ...
        item.check_value = [](const Input_Item& item, const Parameter& para) {
            ...
            if (std::find(allowed.begin(), allowed.end(), para.input.sccs_preset) == allowed.end())
            {
...
#undef ADD_SCCS_REAL_ITEM
    {
        ...
        item.check_value = [](const Input_Item& item, const Parameter& para) {
            ...
            if (std::find(allowed.begin(), allowed.end(), para.input.sccs_boundary) == allowed.end())
            {
    ...
    }
    {
        ...
        item.check_value = [](const Input_Item& item, const Parameter& para) {
            if (para.input.sccs_maxiter <= 0 || !std::isfinite(para.input.sccs_epsilon)
                || para.input.sccs_epsilon < 1.0 || !std::isfinite(para.input.sccs_rho_min)
                || !std::isfinite(para.input.sccs_rho_max) || para.input.sccs_rho_min <= 0.0
                || para.input.sccs_rho_max <= para.input.sccs_rho_min
                || !std::isfinite(para.input.sccs_gamma)
                || !std::isfinite(para.input.sccs_pressure)
                || !std::isfinite(para.input.sccs_mixing) || para.input.sccs_mixing <= 0.0
                || para.input.sccs_mixing > 1.0 || !std::isfinite(para.input.sccs_tol_rms)
                || para.input.sccs_tol_rms <= 0.0 || !std::isfinite(para.input.sccs_tol_max)
                || para.input.sccs_tol_max <= 0.0
                || !std::isfinite(para.input.sccs_surface_eta)
                || para.input.sccs_surface_eta <= 0.0)
            {
